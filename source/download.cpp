...
int download_progress(void *p, double dltotal, double dlnow, double ultotal, double ulnow)
{
    ...
    {
        printf("*** Lade herunter: %.2fMB of %.2fMB ***\r", dlnow / _1MiB, dltotal / _1MiB);
        consoleUpdate(NULL);
    }
...
static size_t WriteMemoryCallback2(void *contents, size_t size, size_t nmemb, void *userp)
{
  ...
  if(ptr == NULL) {
    /* out of memory! */ 
    printf("nicht genuegend Speicherplatz (realloc returned NULL)\n");
    return 0;
  }
...
std::string fetchVersion(const char *url, const char *range){
  ...
  /* check for errors */ 
    std::string ver = "-1";
  if(res != CURLE_OK) {
    fprintf(stderr, "curl_easy_perform() fuer Seitenabruf fehlgeschlagen: %s\n",
